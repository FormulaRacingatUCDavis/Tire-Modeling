...
for p = 1 : size( Raw, 1 )
    for c = 1 : size( Raw, 3 )
        ...
        fsurf( @(Fz, Alpha) Mx( Fz , Mesh(p,1,c).Inclination,...
            Mesh(p,1,c).Pressure, 1 , deg2rad(Alpha) ), [0 2500 -15 15] )
...
function [Mx] = VariantEval( Tire )
        ...
        Ey = @(Fz, Gam, Alpha, Hy) ( Tire.Pacejka.p.E.y(1) + Tire.Pacejka.p.E.y(2).*dFz(Fz) ) .* ...
            ( 1 + Tire.Pacejka.p.E.y(5).*Gam.^2 - ...
            ( Tire.Pacejka.p.E.y(3) + Tire.Pacejka.p.E.y(4).*Gam ).*sign(Alpha + Hy) );

        Fy0 = @(Pi, Fz, Gam, Alpha) Dy(Pi, Fz, Gam) .* ...
            sin( Cy .* atan( (1-Ey(Fz, Gam, Alpha, Hy(Pi, Fz, Gam) )) .* ...
            By(Pi, Fz, Gam).*(Alpha + Hy(Pi, Fz, Gam) ) + ...
            Ey(Fz, Gam, Alpha, Hy(Pi, Fz, Gam) ).*atan( ...
            By(Pi, Fz, Gam).*(Alpha + Hy(Pi, Fz, Gam) ) ) ) ) + Vy(Fz, Gam); 
        ...
        Fx0 = @(Gam, Fz, Pi, Kappa) Dx(Fz,Pi, Gam) .* sin( Cx .* ... 
            atan( (1-Ex(Gam,Fz,Kappa)) .* Bx(Fz,Pi,Gam).*(Kappa + Hx(Fz)) + ...
            Ex(Gam, Fz, Kappa).*atan( Bx(Fz, Pi, Gam).*(Kappa + Hx(Fz)) ) ) ) + Vx(Fz); 
        ...
        % Defining Fy
        Fy = @(Gam, Fz, Pi, Kappa, Alpha) abs( Fx0(Gam, Fz, Pi, Kappa) .* Fy0(Pi, Fz, Gam, Alpha) ./ ...
            sqrt( (Kappa - Kappa0(Fz, Pi)).^2 .* Fy0(Pi, Fz, Gam, Alpha).^2 + ...
            Fx0(Gam, Fz, Pi, Kappa).^2 .* tan(Alpha - Alpha0(Fz, Gam, Pi)).^2 ) .* ...
            sqrt( (1 - abs(Kappa - Kappa0(Fz,Pi))).^2 .* cos(Alpha - Alpha0(Fz, Gam, Pi)).^2 .*...
            Fy0(Pi, Fz, Gam, Alpha).^2 + sin(Alpha - Alpha0(Fz, Gam, Pi)).^2 .* Kxk(Fz,Pi).^2 ) ./ ...
            ( Kxk(Fz,Pi) .* cos(Alpha - Alpha0(Fz, Gam, Pi))) ) .* sign(Fy0(Pi, Fz, Gam, Alpha));           
        
        Mx = @(Fz, Gam, Pi, Kappa, Alpha) (Tire.Pacejka.Ro .* Tire.Pacejka.Fzo) .* ... 
            ( Tire.Pacejka.q.S.x(1) - ((Tire.Pacejka.q.S.x(2) .* Gam) .* ...
            (1 + Tire.Pacejka.p.p.mx .* dPi(Pi))) +(Tire.Pacejka.q.S.x(3) .* ...
            (Fy(Gam, Fz, Pi, Kappa, Alpha)./Tire.Pacejka.Fzo)) + ...
