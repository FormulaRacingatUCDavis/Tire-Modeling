function Tire = CombinedAligningFitting( Tire, Data, Bin )
% Executes all of the fitting procedures for combined slip aligning moment
...
for p = 1 : numel( Case.Pressure )
    for z = 1 : numel( Case.Load )
        for c = 1 : numel( Case.Camber )
            ...
            Nominal(p,z,c) = CombinedAligningNominal( Raw(p,z,c) );
        end
    end
end
...
%% Variant Fitting
Response = CombinedAligningResponseSurfaces( Mesh, Nominal, Tire );
  
[ Variant, Tire ] = CombinedAligningVariant( Raw, Response.x0, Tire ); %#ok<ASGLU>

%% Plotting Function
CombinedAligningPlotting( Mesh, Raw, Nominal, Tire );