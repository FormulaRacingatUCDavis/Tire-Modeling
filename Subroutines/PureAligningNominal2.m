 function [Nominal] = PureAligningNominal2( Mesh, Raw, Tire )
...
%% Fit Type
Fit.Type = fittype( ['-(Dt*cos( Ct*atan((1-Et)*(Bt*(Slip+Ht)) + Et*atan(Bt*(Slip+Ht))))*cosd(Slip)) * ', ... % Pneumatic Trail Function
    ...
    '(Dr*cos(atan(qbz10*', num2str(Fyo.By*Fyo.Cy), '*(Slip+', num2str(Fyo.Hy+Fyo.Vy/Fyo.Kya), ')))*cosd(Slip))'], ...
    ... % Fit Options
...
%% Fit model to data
[Fit.Result, Fit.GoF] = fit( Data.Slip( -11 < Data.Slip & Data.Slip < 11), ...
    Data.Moment( -11 < Data.Slip & Data.Slip < 11), Fit.Type, Fit.Opts );
...
%% Local Functions
    function Fyo = FyoEvaluation( Mesh, Tire )
        ...
        Fyo.Ey(1) = ( Tire.p.E.y(1) + Tire.p.E.y(2).*Mesh.dFz ) .* ...
            ( 1 + Tire.p.E.y(5).*0.^2 + ...
            ( Tire.p.E.y(3) + Tire.p.E.y(4).*0 ) );
        
        Fyo.Ey(2) = ( Tire.p.E.y(1) + Tire.p.E.y(2).*Mesh.dFz ) .* ...
            ( 1 + Tire.p.E.y(5).*0.^2 - ...
            ( Tire.p.E.y(3) + Tire.p.E.y(4).*0 ) );
