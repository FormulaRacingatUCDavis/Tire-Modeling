...
[Bt.Data(:,1), Bt.Data(:,2), Bt.Data(:,3)] = ...
    prepareSurfaceData( [Mesh.dFz], [Mesh.Camber], [Nominal.Bt] );
...
[Ht.Data(:,1), Ht.Data(:,2), Ht.Data(:,3)] = ...
    prepareSurfaceData( [Mesh.dFz], [Mesh.Camber], [Nominal.Ht] );
...
Dr.Surface = @(Pi, Fz, Gam, x0)  Tire.Ro .* Fz .* ( ( x0.qdz6 + x0.qdz7.*dFz(Fz) ) + ...
...
%% Local Functions
    ...
    function MeanSquareError = ErrorDt( qdz1, qdz2, qdz3, qdz4, ppz1 )
        DtSurface = Tire.Ro .* ([Mesh.Load]./Tire.Fzo) .* ...
            ( qdz1 + qdz2.*[Mesh.dFz] ) .* ( 1 - ppz1.*[Mesh.dPi] ) .* ...
            ( 1 + qdz3.*abs([Mesh.Camber]) + qdz4.*[Mesh.Camber].^2 );
    ...
    function MeanAbsoluteError = ErrorEt( qez1, qez2, qez3, qez4, qez5 )
        EtSurface = (qez1 + qez2.*[Mesh.dFz] + qez3.*[Mesh.dFz].^2) .* ...
            (1 + (qez4 + qez5.*[Mesh.Camber]).*(2/pi).* ...
            atan( Bt.Solution([Mesh.dFz], [Mesh.Camber]).*x0.qcz1.*5 ) );
        
        MeanAbsoluteError= mean( abs( [Nominal.Et] - EtSurface ) );
    end
    
    function MeanSquareError = ErrorDr( qdz6, qdz7, qdz8, qdz9, qdz10, qdz11, ppz2 )
        DrSurface = Tire.Ro .* [Mesh.Load] .* ( ( qdz6 + qdz7.*[Mesh.dFz] ) + ...
            ( ( qdz8 + qdz9.*[Mesh.dFz] ) .* ( 1 + ppz2.*[Mesh.dPi] ) + ...
            ( qdz10 + qdz11.*[Mesh.dFz] ) .* abs([Mesh.Camber]) ).*[Mesh.Camber] );
